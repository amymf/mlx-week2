...
query_tower = model.RNNTower(vocab_size=vocab_size, embed_dim=128, hidden_dim=128)
doc_tower = model.RNNTower(vocab_size=vocab_size, embed_dim=128, hidden_dim=128)
...
def process_batch(batch, max_length=20):
    def tokenize_and_pad(texts):
        ...
        lengths = torch.tensor([len(seq) for seq in tokenized]).to(device)
    ...
    query_vec = query_tower(query, query_lengths).to(device)
    pos_vec = doc_tower(pos_doc, pos_doc_lengths).to(device)
    neg_vec = doc_tower(neg_doc, neg_doc_lengths).to(device)
